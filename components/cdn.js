...
class SteamUserCDN extends SteamUserApps {
	...
	getManifest(
		...
		if (
			...
			this._warn(
				`appinfo format has changed: you now need to use appinfo.depots[${depotID}].manifests.${branchName || "public"}.gid to access the manifest ID. steam-user is fixing up your input, but you should update your code to retrieve the manifest ID from its new location in the appinfo structure.`,
			);
		}
